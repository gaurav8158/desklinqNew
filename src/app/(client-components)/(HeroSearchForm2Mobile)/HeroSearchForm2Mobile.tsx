...
import CabinsSearchForm from './(cabins-search-form)/CabinsSearchForm'
...
// export type SearchTab = 'Hot desk' | 'Meeting rooms'
...
const HeroSearchForm2Mobile = () => {
  ...
  useEffect(() => {
    dispatch(updatePrimaryFilter({ space: tabs[selectedIndex] }))
  }, [selectedIndex])
        ...
        <div className="ml-3 flex-1 text-left overflow-hidden">
          ...
          <span className="block mt-0.5 text-xs font-light text-neutral-500 dark:text-neutral-400 ">
            <span className="line-clamp-1">
              {/* Anywhere • Any week • Add Co-workers */}
              Workspace Name or Area/City
            </span>
          </span>
        </div>
                      ...
                      <Tab.List className="pt-12 flex w-full justify-center font-semibold text-sm sm:text-base text-neutral-500 dark:text-neutral-400 space-x-6 sm:space-x-8">
                        {tabs.map((item, index) => (
                          <Tab key={index} as={Fragment}>
                            {({ selected }) => {
                              return (
                                <div className="relative focus:outline-none focus-visible:ring-0 outline-none select-none truncate">
                                  <div
                                    className={`${
                                      selected
                      ...
                      <div className="px-4 py-3 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700 flex justify-between">
                        <button
                          ...
                          onClick={() => {
                            ...
                            dispatch(updatePrimaryFilter({ minCapacity: 1 }))
