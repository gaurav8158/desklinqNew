...
const DetailsDateRange: FC<Props> = ({
  ...
  return (
    <Popover className={`StayDatesRangeInput z-20 relative flex`}>
      {({ open }) => (
        <>
          <Popover.Button
            className={`flex-1 border rounded-2xl border-neutral-300 mb-2 dark:text-neutral-400 flex relative p-3 items-center space-x-3 focus:outline-none ${
              open ? 'shadow-lg' : ''
            }`}
          >
            {renderInput()}
