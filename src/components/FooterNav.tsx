...
const FooterNav = () => {
  ...
  const NAV: NavItem[] = [
    ...
    {
      name: 'Wishlists',
      link: '/account-savelists' as Route,
      icon: HeartIcon,
    },
    {
      name: userData.id ? userData.firstName : 'Log in',
      link: userData.id ? '/account' : ('/auth/login' as Route),
      icon: UserCircleIcon,
      isAvatar: userData.id ? true : false,
    },
    {
      name: 'Menu',
      icon: MenuBar,
  ...
  const renderItem = (item: NavItem, index: number) => {
    ...
    return item.link ? (
      ...
      >
        {item.isAvatar ? ( // Check the isAvatar flag
          <Avatar /> // Replace with your avatar icon
        ) : (
          <item.icon className={`w-6 h-6 ${isActive ? 'text-red-600' : ''}`} />
        )}
        <span
          className={`text-[11px] leading-none mt-1 truncate ${
